...
class Trainer:
    ...
    def _run_batch(self, batch):
        ...
        if self.mixed_precision_dtype == torch.float16:
            ### YOUR CODE HERE ###
            self.gradscaler.scale(loss).backward()  # Scale the loss
        else:
            loss.backward()
    ...
    def _run_epoch(self, train_dataloader, epoch):
        ...
        for step, batch in enumerate(train_progress_bar):
            ...
            if steps % self.gradient_accumulation_steps == 0:
                ...
                if self.mixed_precision_dtype==torch.float16:
                    self.gradscaler.step(self.optimizer)  # Step the optimizer
                    self.gradscaler.update()  # Update the scale for next iteration
                else:
                    self.optimizer.step()
