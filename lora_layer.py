...
class Linear(nn.Linear, LoraLayer):
    ...
    def merge(self):
        ...
        if self.merged:
            ...
            return
        if self.r[self.active_adapter] > 0:
            # Merge the LoRA parameters by adding the product of lora_B weights and lora_A weights (after transposing if necessary)
            # to the original weights, scaled by the LoRA scaling factor.
            self.weight.data += (
                transpose(self.lora_B[self.active_adapter].weight @ self.lora_A[self.active_adapter].weight, self.fan_in_fan_out) 
                * self.scaling[self.active_adapter]
            )
            self.merged = True
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            # If the LoRA adapter is active and not merged, add the output of the LoRA layers to the result.
            result += (
                (self.lora_B[self.active_adapter](self.lora_dropout(self.lora_A[self.active_adapter](x))))
                * self.scaling[self.active_adapter]
            )
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
